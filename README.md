...
![Rust_CLI_가챠](https://github.com/user-attachments/assets/feb727ca-1734-4591-9d2f-6fcba877aab1)
...
1. **Rust 문법 이해**
   - `struct`, `enum`, `match` 등을 사용해보며 JS/TS와 다른 Rust의 타입 사용을 익힘
   - 함수/메서드에 값을 전달할 때 `&mut`을 이용하여 설계
2. **구조체 활용**
   - 보상 종류는 `enum`, 유저 상태는 `struct`로 정의
   - 누적 뽑기 횟수, 메달 수, 코인 잔액 등을 하나의 상태로 묶어서 관리
3. **CLI 입력 파싱**
   - `stdin`으로 문자열을 입력받아 `trim()`, `split_whitespace()` 등으로 파싱
   - `draw 1`, `draw 10`, `exit`와 같은 명령을 조건문으로 처리
4. **로직 분리**
   - 사용자 입력, 뽑기 로직, 코인 계산, 출력 기능을 함수별로 분리
